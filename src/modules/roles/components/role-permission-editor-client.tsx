    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Atur kewenangan setiap peran dan sinkronkan akses pengguna terkait.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
        ...
        <Card>
          <CardHeader className="flex flex-col gap-4 border-b sm:flex-row sm:items-start sm:justify-between">
            <div>
              <CardTitle>
                Matriks Hak Akses — {selectedRole?.name ?? "Peran"}
              </CardTitle>
              <p className="mt-2 text-sm text-muted-foreground">
                Dipilih {selectedPermissions.length} dari{" "}
                {data.permissions.length} hak akses.
              </p>
            </div>
          ...
          <CardContent className="space-y-6 p-6">
            {isSuperAdminLocked ? (
              <div className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                Peran super admin dikunci dan selalu memiliki seluruh hak akses.
              </div>
            ) : null}
                ...
                >
                  <div className="flex flex-col gap-3 border-b px-4 py-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {selectedCount} / {group.permissions.length} dipilih
                      </p>
                    </div>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              Perubahan akan memperbarui peran, menyinkronkan cache akses pengguna,
              dan dicatat dalam riwayat aktivitas.
            </div>
          </CardContent>
        </Card>
      </div>
